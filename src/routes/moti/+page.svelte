<script lang="ts">
    ...
    onMount(() => {
        ...
        channels.forEach(channel => {
            const element = document.querySelector(`.channel-${channel.cssClass}`);
    ...
    <div class="flex flex-row">
        {#each channels as channel}
            <button on:click={()=>setChannel(channel)}
                    class="{`channel-${channel.cssClass}`} {channel.colorClass} text-blue-800 text-xs font-medium me-2 px-2.5 py-0.5 rounded dark:bg-blue-900 dark:text-blue-300">
                {channel.label}
            </button>
        {/each}
    </div>
