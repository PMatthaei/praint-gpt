...
<svelte:head>
    ...
    <meta name="description" content="Connect a Braincell"/>
</svelte:head>

<form class="flex flex-col gap-4 h-screen p-4 sm:ml-64" method="POST" action="?/connect" enctype="multipart/form-data">
    <div class="flex items-center justify-center w-full">
        <label for="dropzone-file"
            ...
            <input id="dropzone-file" type="file" class="hidden" name="file" bind:value={file}/>
        </label>
    </div>
    ...
    <div class="flex flex-col grow w-full p-6 bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
        <div id="editorjs" class="grow w-full"></div>
        <input id="editor-json" type="text" class="hidden" required name="json" bind:value={jsonString}/>

        <div class="flex flex-row justify-center">
            <button data-tooltip-target="tooltip-default" type="submit" disabled="{disable}"
                    class={
                    disable ?
